...
- 로그인 인증 과정을 카카오에 맡김
- 회원가입 절차가 따로 없으므로 처음 로그인 할 때는 회원가입 처리를 하고, 두 번째 로그인부터는 로그인 처리를 해야함
- 카카오 로그인은 성공시 자동으로 req.login 호출해줌
- 기본 URL, redirect URL 설정 필요
- 카카오 전략
        ...
        try {
          ...
          if (exUser) {
            ...
            const newUser = await User.create({
              email: profile._json && profile._jsoin.kakao_account_email,
